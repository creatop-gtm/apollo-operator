...
### Check what the filter actually admits

A signal filter can let in a second population you did not ask for. On 2026-09-14 a `latest_funding_date_range` angle, meant to find companies that had just raised, was **34% companies that had just been acquired**, because Apollo's "latest funding" is the latest funding *event* and an acquisition is one. Pairing it with `organization_latest_funding_stage_cd` fixed it (see `apollo-signals`). The universe size looked reasonable the whole time; only a sample of the companies behind it showed the problem.

So before enriching an angle, look up a handful of the companies it returns and check they carry the signal the angle is named after. Budget for it: a company lookup is 1 lead credit each, and ten of them is cheap next to enriching a list built on the wrong premise.
...
- **Person-id suppression does not stop company overlap.** Two angles can reach the same company through two different people. Apply the per-company cap across every part and angle of the ICP, not within one file: in a live run on 2026-09-14, **47 of the 59 contacts removed by the cap were removed because a higher-priority part already had someone at that company.** And give each campaign on the same ICP its own subject lines, so two colleagues comparing inboxes see two different conversations rather than one blast.
- **Put competitor exclusion in the angle definition when what you sell is something your competitors also buy**, such as data or outbound services. They reply because they are buyers, not because they are your ICP: in the same run, the first positive reply to a data-service campaign came from a competitor. Exclude them at search time, by keyword, NAICS, or a named domain list, not after they reply.
...
**Sending capacity sets the number of angles, not credits.** Every angle queues on the same pool of warmed mailboxes. Sizing is free and enrichment is affordable; the constraint that actually binds is mailboxes multiplied by sends per mailbox per day. A fifth angle on a pool that is already full is a list that waits, and a waiting list decays (see the timing-signal drift rule in `apollo-list-builder`). Do the capacity math before building another angle, not after. The volume math is in `sending-infrastructure`.
...
**An angle cannot rescue the wrong persona either.** A good timing signal aimed at the wrong buyer still returns nothing: in a live run, a recently-hired-or-promoted signal sent to sales leaders, for an offer that founders buy, produced **706 sends and zero positive replies.** Check the persona before blaming the angle, and check the angle before blaming the copy.
