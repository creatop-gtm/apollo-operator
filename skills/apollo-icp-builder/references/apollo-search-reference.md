...
| Tech stack | `currently_using_any_of_technology_uids` / `all_of` | Underscores for spaces, e.g. `google_analytics`. Not always the obvious slug: Apollo itself is `apollo`, and `apollo_io` returns zero. People search can also exclude with `currently_not_using_any_of`. |
...
| Recently funded | `latest_funding_date_range`, `latest_funding_amount_range`, `total_funding_range`, plus `organization_latest_funding_stage_cd` every time, or acquisitions come through | both (date and stage verified on people search 2026-09-14) |
