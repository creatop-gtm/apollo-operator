...
- **Research signals** need a look at the company. Example: "has a pricing page," "sells a specific product line." Apollo's own AI research lives inside its agent (`apollo_agent_find_prospects` escalates to it), which as of 2026-09-16 is reachable only on the v1 MCP with a master API key, so by default Claude does this natively: read the site, decide, tag the record. This runs at list time (List), but define the rule here.
...
Full treatment, including the three kinds of angle and how to sequence several: `references/angles.md`. Starting from a named set of companies instead of an ICP, or wanting several people per company: `account-based-outbound`.

### 6c. Apollo's agent covers this step too. Say so.

Apollo ships its own targeting judgment on the MCP: `apollo_agent_find_prospects` turns a plain-language description into a search and escalates to semantic and AI research, `apollo_agent_plan_gtm_campaign` proposes campaign concepts from the Context Center and sizes each audience live, and `apollo_agent_build_lead_scores` builds a scoring model from the ICP. Tested 2026-09-16: planning read the account's real product profiles, sized four audiences, stopped for confirmation, and cost nothing.

**Offer it, do not decide silently.** When the Context Center is already filled in and the operator wants a fast first cut, Apollo's agent is a legitimate route to steps 3 to 6 and the operator should hear that before you build filters by hand. Reachability is the catch, as of 2026-09-16: it runs only on the v1 MCP with a master API key. The v2 dispatcher returns `not_implemented` and the Claude Code connector does not list it, so check `apollo_find_tools` rather than trusting this paragraph.

**What comes back is a search, not a list.** The agent does not size the attrition between an audience and a sendable list, grade composition, or verify anything. So treat its output as the input to 6b (size it against the other angles), 7 (validate a sample), and then List, exactly as you would a filter set you wrote yourself. The full stance is in `operator-context`.
