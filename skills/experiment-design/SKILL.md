...
6. **Measure after the full sequence has run** (through the last step plus a reply grace period), using `positive-reply-scoring`. Pull each arm's numbers with `campaign-reporting`, grouped by template for copy tests, and keep the send count next to every rate. Measuring early biases toward the first email.
