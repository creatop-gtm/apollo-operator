...
**The same data is on MCP**, for when you are working in conversation rather than on disk: `apollo_conversations_search` to find calls, `apollo_conversations_get_insights` for the summary (outcome, pricing discussion, next steps, objections, pain points, plus timestamped action items), `apollo_conversations_get_transcript` for the full text, and `apollo_conversations_get_recording_links` for the media. Insights only exist once a call reaches `state: insights_generated`. The CLI is the better route for many calls at once, because the output goes to disk.
