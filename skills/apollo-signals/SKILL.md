...
- **Recently funded:** `latest_funding_date_range`, `latest_funding_amount_range`, `total_funding_range`, and **always `organization_latest_funding_stage_cd` with them**. Apollo's "latest funding" is the latest funding *event*, and an acquisition counts. On 2026-09-14 a "funded since June" universe was **34% Merger / Acquisition** (39% of the enriched list), plus debt financing. A founder whose company was just bought is not the buyer a funding angle assumes. Stage codes confirmed by company lookup: `2` Series A, `10` Venture (round not specified), `13` Debt Financing, `16` Merger / Acquisition. Inferred from counts, not confirmed: `0` Seed, `3` Series B, `12` Private Equity. Confirm any other code the same way before relying on it: size the query per code, then look up a few of its companies. The **person-enrich payload carries no funding fields** (no round date, stage, or amount on `organization`), so checking a funding filter costs one company enrichment per company, at 1 lead credit each.
- **Technology adoption:** `currently_using_any_of_technology_uids` (also `_all_of_`, and the `not_` exclusion). "Uses or added tool X." Uids are not always the obvious slug: Apollo itself is `apollo`, and `apollo_io` returns zero.
...
**A wrong parameter name fails silently over REST.** On lane 3, `organization_latest_funding_date_range` and `organization_intent_signals` were both accepted and ignored, returning the unfiltered count. **The tell is a total equal to your baseline.** Size the query without the new filter first, then with it; if the number does not move, the name is wrong, not the market. Names verified on `mixed_people/api_search` 2026-09-14: `latest_funding_date_range`, `organization_latest_funding_stage_cd`, `organization_job_posted_at_range`, and `currently_using_any_of_technology_uids`. The MCP validates against its schema and rejects unknown names instead, so this trap is specific to REST.
...
- **Into targeting (Targeting and List):** a good signal becomes a scoring rule or a filter, so it sharpens the list.
