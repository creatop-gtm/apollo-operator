...
This phase assumes the sending stack already exists. If you have no dedicated domains or mailboxes yet, build them first with `sending-infrastructure` (Infrastructure), then come back here to keep them healthy.
...
Run `apollo_email_accounts_index` (or `apollo email-accounts list` on the CLI). For each connected sending mailbox, confirm:
...
- SPF, DKIM, and DMARC pass for its domain. For mailboxes connected to Apollo, read the stored verdicts with the domain diagnosis tool described in `sending-infrastructure`, and check how old they are.
...
**Read the limit from the mailbox itself.** `apollo email-accounts list` (CLI) returns each mailbox's `email_daily_threshold` (the daily cap), `max_outbound_emails_per_hour`, `seconds_delay_between_emails`, `limits_editable`, and its warmup status. **Do not use the analytics metric `email_daily_limit` as the ceiling:** on 2026-09-14 it reported 775 for a mailbox whose own setting was 25. Use analytics for what was actually sent, and the mailbox record for what is allowed.

**Changing a mailbox limit is a UI job.** No MCP tool or CLI command edits it. To cap one campaign without touching the mailbox, set `max_emails_per_day` on the sequence with `apollo_sequences_update`, and echo every step, touch, and template back, because that update deletes anything you leave out.
...
**Readable per sequence, not settable.** Probed 2026-08-31: `bounce_guard`, `bounce_guards`, `email_bounce_guard`, `settings/bounce_guard`, `email_deliverability_settings`, and `team_settings` all return 404 on REST, and there is no dedicated MCP tool. **But since 2026-09-14 every sequence object returned by the MCP** (`apollo_sequences_create`, `apollo_sequences_update`, `apollo_emailer_campaigns_show`) carries `auto_pause_enabled` and `auto_pause_config`, for example `{"evaluation_window_days": 7, "min_volume": 200, "warning_threshold_pct": 3.0, "auto_pause_threshold_pct": 4.0}`. So an agent can confirm the guard is on for a given sequence and read its thresholds before activation. Changing them headlessly is still unverified, so **set them in the UI as part of onboarding any account**, then read them back from the sequence.
...
- **Keep backup infrastructure ready.** Run spare mailboxes, warmed and waiting: a few for an in-house team, closer to double for a large program or an agency. When something degrades you swap instead of stopping. This is the lesson that separates operators who scale from operators who stall.
