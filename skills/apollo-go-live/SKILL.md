...
- **Check the opt-out before the first enrollment.** Confirm the workspace unsubscribe link is on (a UI setting, not exposed through the API) and that the signature carries a physical address. See `compliance.md`.
...
Read the sequence back first with `apollo_emailer_campaigns_show`, so the summary describes what is actually saved (step count, subjects, and the opening of each body), not what you meant to build. Then show the operator a plain summary and stop:
...
**Then confirm it is sending.** A day in, `apollo_emailer_messages_search` with the sequence id and `display_mode: "metadata_mode"` returns the counts the UI shows (scheduled, sent, delivered, bounced, replied). Scheduled but not sent inside the send window is the thing to catch early; read the schedule section below before calling it broken. **Nothing scheduled at all, with contacts enrolled, usually means the touches are not approved.** Read the sequence with `apollo_emailer_campaigns_show` and check every touch's `status`: only `approved` sends, and `to_be_reviewed` holds the whole sequence silently. Verified 2026-09-14 on a sequence that sat active for two weeks with five contacts and zero sends.
...
**Use the CLI or REST for this, not MCP.** The Apollo UI, the CLI, and REST (`POST /emailer_campaigns/{sequence_id}/abort`) all stop a sequence in one action. MCP can too, through `apollo_sequences_update` with `active: false`, but that update is declarative: any step you do not echo back is deleted. Mid-incident is the wrong moment to get that right. The safe MCP version is in `apollo-operator`.
...
- **Activating without a kill switch installed.** `sequences abort` is one call on the CLI and REST. The MCP route exists but deletes steps if done carelessly. Get the CLI working before go-live, not in the middle of an incident.
