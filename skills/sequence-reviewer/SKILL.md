...
- **Voice:** no em dashes, no buzzwords (leverage, synergy, revolutionary, best-in-class, game-changing), short paragraphs. Those two read as machine-written to a prospect. Punctuation and capitalisation style beyond that is the operator's house style, not this library's business: do not flag it.
...
### Personalization
- **Every merge field has a fallback.** Flag any variable with no default. A contact whose first name is missing gets "Hi ," and a missing company gets a sentence with a hole in it, which is the most obviously automated thing an email can do. Check the list too: if a field is empty for part of the list, either set a fallback that reads naturally or cut those records from the send.
