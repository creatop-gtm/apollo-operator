...
- **Word count by step: 60 to 75 for step 1, 45 to 60 for step 2, and step 3 the shortest of the three: one or two sentences, three at most.** Step 3 asks a routing question and closes the loop. It does not need setup, and length works against it.
...
- **Plain text. No em dashes. No buzzwords** (leverage, synergy, revolutionary, best-in-class). Both read as machine-written. Beyond those two, write in the operator's house style rather than imposing one.
- **Give every merge field a fallback.** A contact with no first name must not receive "Hi ,". Set a default that reads naturally, or exclude the records missing that field.
...
**You can read the copy back now, but not on every lane.** `apollo_emailer_campaigns_show` (MCP) returns every step, touch, subject, and body, and `GET /emailer_campaigns/{id}` returns the same with a master API key, both verified 2026-09-14. The CLI lanes still cannot: the create response, `apollo sequences search`, and the MCP campaign search all report `touches: 0` on every step, and `GET /emailer_campaigns/{id}` returns **403** on the CLI's OAuth token. So after a CLI create, read the sequence back over MCP before anything else.

**A human still opens the sequence before activation.** Reading the bodies back proves they landed; it does not approve them. Do not treat a successful create, or a clean read-back, as sign-off. The failure mode is sending wrong-bodied emails to a real list, and it is unrecoverable once sent.

### Always hand back the sequence URL. Every create, every update.

Because approval happens by eye, **the operator needs a link, not an id.** End every create and every update by printing it:
...
(An agent can now summarise the saved sequence from `apollo_emailer_campaigns_show` next to the link, which makes the human check faster. It does not replace it.)
...
Read them from `apollo_emailer_campaigns_show` (MCP), which returns every step, touch, and template id with its content. `apollo sequences search` expands `emailer_steps` with `id`, `position`, `type`, `wait_mode`, and `wait_time` but never the touches, so on the CLI alone step structure is readable and step content is not.

Two consequences. On the CLI, keep the steps payload in a local file as the source of truth, because the CLI cannot read the bodies back; on MCP, read them with `apollo_emailer_campaigns_show` first. Either way, re-send **every** step on every update, with its id, since an omitted step is a deleted step.
...
**A third option is in the schema, and did not work when tested.** `apollo_sequences_create` and `update` accept `generation_options` on a touch, with types `ai_full_email`, `ai_follow_up_email`, and `ai_last_pitch_email`, meant to have Apollo write the whole body, plus `template_type: "prompt_template"` for a reusable prompt. **Tested 2026-09-14:** a create with `ai_full_email` and a matching `{{ai_email_body}}` merge variable in the body was accepted, but the saved touch came back with `generation_options: []` and `has_ai_variables: false` on both the create response and an independent `apollo_emailer_campaigns_show`. The option was silently dropped, nothing was generated, and no `ai_credit` was spent. It may need an account feature this test account lacks. **Always read the touch back after setting it**, because a create that succeeds tells you nothing about whether the AI option survived.

### Apollo's agent can draft the whole sequence. Offer it.

`apollo_agent_draft_sequence_copy` is Apollo's own drafting, review, and optimisation of sequence copy, grounded in the Context Center. It overlaps this skill directly, so say the option out loud rather than writing by hand as if it did not exist: an operator with a filled Context Center who wants a first draft fast may prefer it, and that is their call. Reachability, as of 2026-09-16: it runs only on the v1 MCP with a master API key, not through the v2 dispatcher and not in the Claude Code connector, so check `apollo_find_tools` first.

Whatever it produces is a draft, not an approved sequence. It goes through `sequence-reviewer` like any other copy, the word bands and the step 3 rule still apply, and a human still approves before activation. Apollo's copy knows the product; it does not know the deliverability history of the mailboxes it will send from. Stance: `operator-context`.

## Editing a live sequence: the diff is destructive

**Kill switch, verified end to end 2026-08-31, on a live sequence with contacts enrolled.** `apollo sequences abort --id <id>` flips a running sequence from `active: true` to `active: false` immediately, and returns the campaign object with a `contact_statuses` breakdown so you can see what was in flight. It does **not** archive, so the sequence and its enrollment survive: reactivating with `sequences update --steps-file <file> --active` resumes it, and the remaining steps continue on schedule.

On a sequence that is already inactive it refuses cleanly with `422 The sequence is already inactive.` rather than erroring obscurely or silently succeeding, so it is safe to call speculatively when you are unsure of the state.

**Prove it before you need it.** Run abort against the real sequence, confirm `active` flips, then reactivate. Two minutes, and it turns the kill switch from something you believe in into something you have used. During an actual incident is the wrong time to discover the command needs a flag you do not have.

**MCP can stop a sequence too, verified 2026-09-14.** `apollo_sequences_update` with `active: false` flips a running sequence to `status_reason: manual_pause`, as long as every step, touch, and template is echoed back with its id. The diff rules below are exactly why that is dangerous, so read the sequence with `apollo_emailer_campaigns_show` first, and prefer `abort` in a real incident.
