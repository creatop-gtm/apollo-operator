...
- **Create work in bulk:** `apollo_tasks_bulk_create` to drop a task on a whole segment, for example a `call` task on every High-priority lead who opened three times. Each task needs `user_id` (the assignee), `type`, and at least one association (`contact_id`, `account_id`, or `opportunity_id`). Put call scripts in `note`; put LinkedIn or email content in `standalone_outreach_task_message`. **Before a call task is worked, check the number is not on a do-not-call registry:** Apollo's DNC screening blocks flagged numbers in its own dialer, but a number called from a personal phone is not blocked. See `compliance.md`.
...
On the CLI: `apollo emails draft`, `apollo emails send` (sends a real email), and `apollo emails status`.
...
| Did they open, click, or reply? | `apollo_emailer_messages_search` with message `ids` (MCP), or `apollo emails search --stats opened replied` (CLI) | Per-message `opened`, `num_opens`, `clicked`, `replied`, `bounced`, plus an event timeline |
| How is a whole sequence doing? | `apollo_emailer_messages_search` with `emailer_campaign_id` and `display_mode: "metadata_mode"` | The status counts the UI shows: sent, delivered, opened, clicked, replied, bounced, spam blocked |
| What have we sent them? | `apollo_emailer_messages_get_content` (MCP) | Subject and body of your own sent emails. **Customer replies are not included**, on any lane |
| What happened to this contact in a sequence? | `apollo_emailer_campaigns_activity_feed` (MCP) | Enrolled, paused, resumed, replied, finished, in order |

All of these are on MCP now. Earlier versions of this table said opens and sent history were REST only; that changed.
