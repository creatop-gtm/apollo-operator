...
For the counts around the replies (sent, bounced, and reply rate by sequence, step, and variant), run `campaign-reporting` first. This skill is the part a report cannot do: deciding which replies are actually positive.

Pull reply data from Apollo, which carries more than it used to (verified 2026-09-14):

- **Which emails got a reply, with Apollo's own read of it.** `apollo emails search --stats replied` (CLI) returns each replied message with a `reply_class`, such as `willing_to_meet` or `follow_up_question`, or no class at all. On MCP, `apollo_emailer_messages_search` gives per-sequence reply counts and per-message `replied` flags, and `apollo_analytics_sync_report` gives campaign-level numbers.
- **Treat `reply_class` as a first pass, not the verdict.** It is Apollo's classification, some replies carry none, and its classes do not map one to one onto the buckets below. Use it to sort the queue, then classify yourself.
- **The reply text itself is not reachable.** `apollo_emailer_messages_get_content` returns your own sent emails and explicitly excludes customer replies. So ask the operator to paste or export the reply bodies: the value of this skill is the classification and the metric, not the fetch.
